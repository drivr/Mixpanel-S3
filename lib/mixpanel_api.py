...
from mixpanel_query.utils import _tobytes, _totext

__all__ = ('Mixpanel',)

class Mixpanel(object):
    """
    Format, send to and parse http responses from the Mixpanel API.
    """
    ...
    def raw_request(self, base_url, method_name, params, response_format):
        ...
        if 'sig' in params:
            del params['sig']
        params['sig'] = self.hash_args(params, self.api_secret)
        ...
        return url_request.urlopen(
            request_url,
            timeout=self.DEFAULT_TIMEOUT if self.timeout is None else self.timeout
        )
